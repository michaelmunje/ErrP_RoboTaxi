    ...
    def act(self, observation, reward):
        ...
        if self.agents[self.curr_agent].env_changed:            
            if self.num_updates == 0 or random.random() < 0.8: 
                ...
                if b.size == 0:
                    # pick a policy: keep previous, default, or raise
                    # self.curr_agent = getattr(self, "curr_agent", 0)
                    raise ValueError("belief is empty")

                valid = np.isfinite(b)
                if not np.any(valid):
                    # self.curr_agent = getattr(self, "curr_agent", 0)
                    raise ValueError(f"belief has no finite entries: {self.belief}")

                max_val = np.nanmax(b)
                candidates = np.flatnonzero(valid & np.isclose(b, max_val, rtol=1e-8, atol=1e-12))

                if candidates.size == 0:
                    # fallback to argmax over valid entries
                    candidates = np.array([np.flatnonzero(valid)[np.argmax(b[valid])]])

                self.curr_agent = int(random.choice(candidates)) 
