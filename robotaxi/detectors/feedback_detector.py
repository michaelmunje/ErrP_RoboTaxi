...
class FeedbackDetector(threading.Thread):
    ...
    def __init__(self, participant_id):
        ...
        self.feature_columns = ['success', 'AU01_c', 'AU02_c', 'AU04_c', 'AU05_c', 'AU06_c', 'AU07_c', 'AU09_c', 'AU10_c', 'AU12_c', 'AU14_c', 'AU15_c', 'AU17_c', 'AU20_c', 'AU23_c', 'AU25_c', 'AU26_c', 'AU28_c', 'AU45_c',  'AU01_r', 'AU02_r', 'AU04_r', 'AU05_r', 'AU06_r', 'AU07_r', 'AU09_r', 'AU10_r', 'AU12_r', 'AU14_r', 'AU15_r', 'AU17_r', 'AU20_r', 'AU23_r', 'AU25_r', 'AU26_r', 'AU45_r', 'pose_Tx', 'pose_Ty', 'pose_Tz', 'pose_Rx', 'pose_Ry', 'pose_Rz']
        self.processed_data = [] 
        self.roll = None
        self.pitch = None
        self.yaw = None 
    ...
    def run(self):
        ...
        for data_file in data_files:
            if data_file.endswith('.csv'):
                ...
                break        
        data_ct = 0
        # check if file is just "0" with no data
        if os.path.getsize(feature_data_file) < 10:
            print("No data in feature file yet")
            return

        # Keep blanks as blanks (NaN) instead of converting/filling them
        df = pd.read_csv(feature_data_file, keep_default_na=True)
        ...
        for row_idx, row in df.iterrows():
            # Convert just the selected columns to numeric; blanks stay as NaN
            feat_series = pd.to_numeric(row[self.feature_columns], errors="coerce")  # keeps NaN for blanks/non-numeric
            feat_data = feat_series.to_list()

            # Debug (commented out): print row index + values (NaNs will show as nan)
            # print(f"[row {row_idx}] " + ", ".join(f"{k}={v}" for k, v in zip(feature_columns, feat_data)))

            # success is first entry; treat NaN as "not successful" (adjust if you prefer)
            success = feat_data[0]
            if (pd.isna(success) or success == 0) and len(self.processed_data) > 0:
                self.processed_data.append(self.processed_data[-1])
                while len(self.processed_data) > buffer_size:
